...
3. 懒汉式单例
    ...
    那么该怎么解决这个问题呢，最简单粗暴的方法是在`getInstance()`方法前加上`synchronized`关键字，使获取单例的方法只有一个线程能访问。但是这么做的话，会出现性能问题，在高并发的场景下会导致大量的线程阻塞，导致系统性能下降。双重检查锁机制便可以解决线程安全和性能问题，代码如下所示。第一个`if`判断语句的作用是单例已经创建好的情况下，线程不用在进入同步块了。第二个`if`判断语句
